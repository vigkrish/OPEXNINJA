    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur dark:border-slate-800 dark:bg-slate-950/90">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="default" size="sm" className="hidden sm:inline-flex" onClick={() => { window.location.href = '/assessment'; }}>Start Diagnostic</Button>
      ...
      {open && (
        <nav className="border-t border-slate-200 px-6 py-4 dark:border-slate-800 md:hidden">
          {links.map(([label, href]) => <a key={label} href={href} onClick={() => setOpen(false)} className="block py-3 font-medium">{label}</a>)}
          <a href="/assessment" onClick={() => setOpen(false)} className="block py-3 font-semibold text-blue-600">Start Diagnostic</a>
          <a href="/contact" onClick={() => setOpen(false)} className="block py-3 font-semibold text-orange-500">Book Discovery Call</a>
        </nav>
      )}
    </header>
  );
